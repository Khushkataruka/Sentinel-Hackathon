...
def test_triple_riding_needs_three_occupants():
    def occupants(n):
        riders = [
            RiderBox(i, (i * 60, 0, i * 60 + 50, 200), helmet=True, helmet_conf=0.8, score=0.9)
            for i in range(1, n + 1)
        ]
        return detector()(
            CROP,
            vehicle_class="motorcycle",
            permitted=["triple_riding"],
            track_context={"rider_boxes": riders},
        )
...
def test_nothing_outside_the_permitted_list_is_ever_asserted():
    ...
    riders = [
        RiderBox(i, (i * 60, 0, i * 60 + 50, 200), helmet=False, helmet_conf=0.7, score=0.9)
        for i in range(1, 4)
    ]
    findings = detector()(
        CROP,
