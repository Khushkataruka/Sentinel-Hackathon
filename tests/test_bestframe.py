...
def make_track(history):
    track = Track(
        track_id=1,
        cls="car",
        kalman=KalmanBox(xyxy_to_xyah(history[0][1])),
...
def test_sharper_crop_wins_at_equal_size():
    ...
    assert bestframe.sharpness(best.crop) > bestframe.sharpness(blurry_image()[150:270, 200:360])
...
def test_two_wheeler_crop_includes_space_above_the_box():
    ...
    bike = Track(
        track_id=1,
        cls="motorcycle",
        kalman=KalmanBox(xyxy_to_xyah(box)),
        state=TrackState.CONFIRMED,
        hits=4,
        history=list(history),
    )
